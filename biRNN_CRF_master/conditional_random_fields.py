...
def log_sum_exp(z, axis=1, name=None):
    with ops.name_scope(name, "log_sum_exp", [z, axis]):
        ...
        # change tf.log() to tf.compat.v1.log()
        return zmax + tf.compat.v1.log(tf.reduce_sum(
...
def logZ(nu_alp, nu_bet, index=0, time_major=False, name=None):
    with ops.name_scope(name, "logZ", [nu_alp, nu_bet, index, time_major]):
        ...
        # removed squeeze_dims=0 argument
...
def log_likelihood(y, f, g, nu_alp, nu_bet, sequence_length=None,
    ...
    with ops.name_scope(name, "log_likelihood",
        ...
        # removed argument dim=2 to axis=2
        ...
        # removed argument dim=3 to axis=3
        y_i = tf.expand_dims(tf.slice(y, [0, 0, 0], [f_seq_len-1, -1, -1]), axis=3)
        
        # removed argument dim=2 to axis=2
        y_plus = tf.expand_dims(tf.slice(y, [1, 0, 0], [-1, -1, -1]), axis=2)
...
def log_marginal(nu_alp, nu_bet, index_start=None, num_index=None,
    ...
    with ops.name_scope(name, "log_marginal",
        ...
        # remove argument dim=1 to axis=1
        z_term = tf.expand_dims(logZ(nu_alp, nu_bet, time_major=True), axis=1)
        ...
        if not time_major:
            ...
            # changed from if len(res.get_shape()) == 3:
            if tf.shape(res) == 3:
